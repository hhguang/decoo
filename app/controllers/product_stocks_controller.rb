class ProductStocksController < ApplicationController
  ...
  def in_out_query    
    result = GoodsStockItem.joins(:goods_stock).where("goods_stocks.type = 'ProductStock' ")    
    
    if params[:search] && !params[:search].empty?
      result = result.where("goods_stocks.bh = ? ", params[:search] )        
    end
    if params[:start] && !params[:start].empty?
      result = result.where("goods_stock_items.created_at > ?", params[:start])
    end
    if params[:end] && !params[:end].empty?
      result = result.where("goods_stock_items.created_at < ?", params[:end])
    end
    if params[:act]
      @act = params[:act]
      result = result.where(:act_type => params[:act])
      @quantity_list =result.group(:bh).sum(:quantity)      
    end
